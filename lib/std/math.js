    ...
    var evaluateMethod = function (name, paramCount, returnType) {
        ...
        return function (result, args, ctx) {
            ...
            for(var i = 0; i < paramCount; i++ ) {
                if(!args[i].canNumber()) {
                    throw new Error("Parameter " + (i + 1) + " has invalid type for Math." + name + ", expected 'number', but got " + param.getType());
                }
            }
    ...
    var MathObject = {
        ...
        /*clamp: {
            ...
            evaluate: function (result, args) {
                ...
                throwError(result.node, "Math.clamp not supported with argument types: " + args.map(function (arg) {
        ...
        smoothstep: {
            ...
            evaluate: function (result, args, ctx) {
                ...
                throwError(result.node, "Math.smoothstep not supported with argument types: " + args.map(function (arg) {
                    return arg.getTypeString();
                }).join(", "));
            },
        ...
        step: {
            ...
            evaluate: function (result, args, ctx) {
                ...
                throwError(result.node, "Shade.step not supported with argument types: " + args.map(function (arg) {
                    return arg.getTypeString();
                }).join(", "));
            },
