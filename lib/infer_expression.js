...
var TYPES = C.TYPES, ERROR_TYPES = Error.ERROR_TYPES;
...
var handlers = {
        ...
        elements.forEach(function (element, index) {
            if (!index) {
                ...
                if (!elementType.setCommonType(elementType, element)) {
                    result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "shade.js does not support inhomogenous arrays: [", elements.map(function (e) {
    ...
    ExpressionStatement: function (node, scope) {
        var expression = scope.type(node.expression);
        ANNO(node).copy(expression);
    },
    ...
    NewExpression: function (node, scope) {
        ...
        if (entry && entry.hasConstructor()) {
            ...
            result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.REFERENCE_ERROR, node.callee.name, "is not defined"));
        }
    },
        ...
        switch (operator) {
            ...
            case "-":
                if (argument.canInt()) {
                    ...
                    result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.NAN_ERROR));
                }
                break;
            case "typeof":
                result.setType(TYPES.STRING);
            ...
            case "delete":
            default:
                result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, operator, "is not supported."));
        }
        if (argument.hasStaticValue()) {
            result.setStaticValue(evaluator.getStaticValue(node, argument.getStaticValue()));
        } else {
            result.setDynamicValue();
        }
    },
    ...
    BinaryExpression: function (node, scope) {
        ...
        switch (operator) {
            ...
            case "%":
                ...
                // number 'op' null => number
                    ...
                    result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.NAN_ERROR, message));
                }
                break;
            case "===":
            case "!==":
        ...
        if (left.hasStaticValue() && right.hasStaticValue()) {
            result.setStaticValue(evaluator.getStaticValue(node, left.getStaticValue(), right.getStaticValue()));
        } else {
            result.setDynamicValue();
        }
    },
    ...
    MemberExpression: function (node, scope) {
        ...
        if (node.computed) {
            if (objectAnnotation.isArray()) {
                ...
                resultType.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "no array access to object yet"));
                return;
        ...
        if (!objectOfInterest.isValid() || objectOfInterest.getType() == TYPES.UNDEFINED) {  // e.g. var a = undefined; a.unknown;
            resultType.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.TYPE_ERROR, "Cannot read property '" + propertyName + "' of undefined"));
            return;
        }
    ...
    CallExpression: function (node, scope) {
        ...
        if (!args.every(function (arg) {
            ...
            result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "Not all arguments types of call expression could be evaluated"));
            return;
        }
        // Be on the safe side, assume result is static independently of former annotations
        result.setDynamicValue();
        ...
        if (node.callee.type == Syntax.MemberExpression) {
            ...
            if (!memberExpression.isFunction()) { // e.g. Math.PI()
                if (objectReference.hasProperty(propertyName)) {
                    result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.TYPE_ERROR, "Property '" + propertyName + "' of object #<" + objectReference.getTypeString() + "> is not a function"));
                    Error.throwError(node, memberExpression.getTypeString() + " is not a function")
                } else {
                    var msg = (object.type == Syntax.ThisExpression ? "'this'" : objectReference.getTypeString()) + " has no method '" + propertyName + "'";
                    result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.TYPE_ERROR, msg));
                    Error.throwError(node, "undefined is not a function");
                }
                return;
            }
            ...
            try {
                ...
                result.setInvalid(Error.generateErrorInformation(node, e.message));
                return;
            }
            ...
            if (!func) {
                result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.REFERENCE_ERROR, functionName, "is not defined"));
                return;
            }
            if (!func.isFunction()) {
                result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.TYPE_ERROR, func.getTypeString(), "is not a function"));
                return;
            }
            try {
                ...
                result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "Failure in function call: ", e.message));
            }
            return;
        }
        result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "Internal:", "Unhandled CallExpression", node.callee.type));
    },
    ...
    LogicalExpression: function (node, scope) {
        ...
        if (operator === "||") {
            ...
            /*if (rightBool === true) {
                // TODO(ksons): We might have a polymorphic type, but a static truth value: true
                return;
            } */
        } else if (operator === "&&") {
            if (leftBool === false) {
            ...
            /*if (rightBool === false) {
                // TODO(ksons): We might have a polymorphic type, but a static truth value: false
                return;
            }*/
        }
        ...
        result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "Can't evaluate polymorphic logical expression"));
    },
    ...
    ConditionalExpression: function (node, scope) {
        ...
        if (testResult === true) {
            ...
            if (result.setCommonType(consequent, alternate)) {
                ...
                result.setInvalid(Error.generateErrorInformation(node, ERROR_TYPES.SHADEJS_ERROR, "Can't evaluate polymorphic conditional expression"))
            }
        }
